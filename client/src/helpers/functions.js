...
/* TODO: user preference set for GHIN HDCP or Course HCAP */
...
export function assignPlayerHandicap(player, card, blankCard) {
  ...
  while (handicapTemp > 0) {
    ...
    const minIndexOrig = card.teeboxes[teebox.value].hdcp.indexOf(minVal);
    ...
    if (hdcpTemp.length === 0) hdcpTemp = [...card.teeboxes[teebox.value].hdcp];
  }
...
export function calculateSkinsIndividual(state, participants) {
  ...
  state.card.holes.forEach((h, hole_index) => {
    ...
    let lowScores = Object.values(holeScores).filter((s) => s === lowScore);
...
function calculateNassauByIndex(
  startIndex,
  endIndex,
  scoring,
  scores_a,
  ...
  for (let ii = startIndex; ii < endIndex; ii++) {
    ...
    if (scoring === "stroke") {
      ...
      const difference_match =
        difference_stroke > 0 ? 1 : difference_stroke < 0 ? -1 : 0;
...
export function calculateNassauIndividual(state, match) {
  const { matchFormat, participants, presses } = match;
  const results = [];
  const scores = [];
  const scoring = matchFormat.value.includes("stroke") ? "stroke" : "match";
  ...
  for (let i = 0; i < scores.length - 1; i++) {
    for (let j = i + 1; j < scores.length; j++) {
      ...
      const matchName = player_a + " vs " + player_b;
      ...
      const { f, b, t } = calculateNassauByIndex(
        0,
        holes,
        scoring,
        scores_a,
      ...
      matchStatus.f.push(f);
      matchStatus.b.push(b);
      matchStatus.t.push(t);

      const matchPresses = presses[matchName];
      const matchHoles = Object.keys(matchPresses || []);

      matchHoles.forEach((h) => {
        const matchType = matchPresses[h];
        ...
        if (matchType === "f") matchStatus.f.push(f);
        if (matchType === "b") matchStatus.b.push(b);
        if (matchType === "t") matchStatus.t.push(t);
      });
...
export function calculateNassauTeams(state, match) {
  const { matchFormat, teams, presses } = match;
  const results = [];
  ...
  teamNames.forEach((team_name, team_index) => {
    ...
    state.card.holes.forEach((h, hi) => {
      ...
      players.forEach((p, pi) => {
        ...
        else if (playerScore < lowHoleScore) lowHoleScore = playerScore;
      });
