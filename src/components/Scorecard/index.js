    ...
    <table>
      <thead className="sticky-row">
          ...
          {props?.state?.nines?.map((n, i) => {
            return (
              <React.Fragment key={i + "HOLE"}>
                {props?.state?.selected_course[n]?.holes?.map((h, i) => <th key={i} id={h === props?.state?.current_hole ? "current_hole" : ""} className="text-white">{h}</th>)}
          ...
          {props?.state?.nines?.map((n, i) => {
            return (
              <React.Fragment key={i + 'HDCP'}>
                {props?.state?.selected_course[n]?.hdcp?.map((h, i) => <th key={i} className="bg-primary text-white">{h}</th>)}
        ...
        <tr>
          ...
          {props?.state?.nines?.map((n, i) => {
            let total = props?.state.selected_course[n].yards?.reduce((t, c) => { return t + c })
            yardageTotal += total;
            return (
              <React.Fragment key={i + 'YARDS'}>
                {props?.state?.selected_course[n]?.yards?.map((h, i) => <th key={i} className="bg-warning">{h}</th>)}
        ...
        <tr>
          ...
          {props?.state?.nines?.map((n, i) => {
            let total = props?.state.selected_course[n].par?.reduce((t, c) => { return t + c })
            parTotal += total;
            return (
              <React.Fragment key={i + 'PAR'}>
                {props?.state?.selected_course[n]?.par?.map((h, i) => <th key={i} className="bg-light-tr">{h}</th>)}
      ...
      <tbody>
        {props?.state?.players?.map((p, i) => {
          let total = 0;
          let bg = (i + 2) % 2 === 0 ? "bg-white" : "bg-light-dark"

          return (
            <tr key={i}>
              <td className={`sticky ${bg} text-dark`}>{p?.name}</td>
              {props.state?.nines?.map((n, nines_index) => {
                let nineTotal = p[n]?.reduce((t, c) => { return t + c })
                total += nineTotal;
                return (
                  <React.Fragment key={i + 'PLAYER' + nines_index}>
                    {p[n]?.map((h, i) => {
                      return (<td key={i} className={`player-score ${bg}`} >{h}</td>) //onClick={() => { props.handleScoreEntry(h, p, n, i) }}
                    })}
                    <td key={i} className={bg} >{nineTotal}</td>
                  </React.Fragment>
                )
              })}
              <td key={i} className={bg}>{total}</td>
